...
*
*/
class DashboardCalidadController extends Controller 
{
	 /**
	...
	public function indicadorCalidad()
	{
		...
		$indicadores = DB::select("select id, color, codigo, nombre from Indicador where categoria='CALIDAD' order by codigo");
		...
		foreach($indicadores as $item)
		{
			...
			$sql = "select id, evaluacion, indicador, codigo, color, total_exp as total, 
				((sum(cumple_exp) / count(cumple_exp)) * 100) as porcentaje, 
				DATE_FORMAT(fechaEvaluacion, '%Y-%m-%d') as fecha, dia, mes, anio, clues, nombre, jurisdiccion, municipio, localidad,
				cone from ReporteCalidad 
				where id = '$item->id' $parametro 
				group by indicador, anio, mes order by fechaEvaluacion ASC";
	...
	public function indicadorCalidadDimension()
	{
		...
		if($nivel == "anio")
			$parametro = "";
		$nivelD = DB::select("select distinct $nivel from ReporteCalidad where clues in ($cluesUsuario) $parametro $order");		
		if($nivel == "month")
		{
	...
	public function topCalidadGlobal()
	{
		...
		$sql = "((select sum(promedio_exp) from ReporteCalidad where clues = c.clues)/(select count(promedio_exp) from ReporteCalidad where clues = c.clues))";
