...
/**
* Controlador EvaluacionPC criterio (PC)
* 
* @package    CIUM API
...
* @created    2015-07-20
*
* Controlador `PC`: Proporciona los servicios para el manejos de los datos de la evaluacion
*
*/
class EvaluacionPCCriterioController extends Controller 
{
	...
	public function index()
	{
		...
		if(array_key_exists('pagina',$datos))
		{
			...
			if(array_key_exists('buscar',$datos))
			{
				...
				$valor   = $datos['valor'];
				$evaluacionCriterio = EvaluacionPCCriterio::orderBy($order,$orden);
				...
				$evaluacionCriterio=$evaluacionCriterio->whereNested(function($query) use ($keyword)
				{
						...
						$query->Where('idEvaluacionPC', 'LIKE', '%'.$keyword.'%')
			...
			{
				$evaluacionCriterio = EvaluacionPCCriterio::skip($pagina-1)->take($datos['limite'])->orderBy($order,$orden)->get();
				$total=EvaluacionPCCriterio::get();
			}
			
		}
		else
		{
			$evaluacionCriterio = EvaluacionPCCriterio::get();
	...
	public function store()
	{
        ...
        try 
		{
			...
			$evaluacionCriterio = EvaluacionPCCriterio::where('idEvaluacionPC',$datos->get('idEvaluacionPC'))->where('idCriterio',$datos->get('idCriterio'))->first();
				
			if(!$evaluacionCriterio)
				$evaluacionCriterio = new EvaluacionPCCriterio;
			
            $evaluacionCriterio->idEvaluacionPC= $datos->get('idEvaluacionPC');
	...
	public function show($evaluacion)
	{
		$indicatores = DB::select("select i.id,i.color,i.codigo,i.nombre from EvaluacionPCCriterio erc
						...
						where erc.idEvaluacionPC = $evaluacion and i.borradoAl is null and erc.borradoAl is null order by i.codigo");
						
		$evaluacionC = DB::table('EvaluacionPC AS e')
			...
			->select(array('e.fechaEvaluacion', 'e.cerrado', 'e.id','e.clues', 'c.nombre', 'c.domicilio', 'c.codigoPostal', 'c.entidad', 'c.municipio', 'c.localidad', 
			'c.jurisdiccion', 'c.institucion', 'c.tipoUnidad', 'c.estatus', 'c.estado', 'c.tipologia', 'mi.nombre as microred', 'me.nombre as mesored', 'ma.nombre as macrored'))
            ->where('e.id',"$evaluacion")
			->where('e.borradoAl',null)
			->first();
			
		$microred = $evaluacionC->microred;
		...
		foreach($indicatores as $indicator)
		{
			...
			WHERE cic.idCone = 0 and ic.idIndicador = $indicator->id 
			...
			foreach($criterio as $valor)
			{
				...
				$aprobado = DB::select("SELECT aprobado from EvaluacionPCCriterio where idEvaluacionPC = $evaluacion
			...
			left join Accion a on a.id = h.idAccion WHERE h.idEvaluacion= $evaluacion and categoriaEvaluacion='PC' and idIndicador = $indicator->id and h.borradoAl is null");
			if($hallazgo)
		...
		foreach($indicatores as $item)
		{
			if(!array_key_exists($item->codigo,$estadistica))
			{
				...
				$total = DB::select("SELECT c.id, c.nombre, c.tipo  FROM ConeIndicadorCriterio cic							
						...
						left join LugarVerificacion lv on lv.id = ic.idlugarVerificacion		
						WHERE cic.idCone = 0 and ic.idIndicador = '$id' 
	...
	public function destroy($id)
	{
        ...
        try 
		{
			$cerrado = EvaluacionPC::where("id",$id)->where("cerrado",null)->orWhere("cerrado",0)->first();			
			if($cerrado)
			{
				$evaluacion = EvaluacionPCCriterio::where("idEvaluacionPC",$id)->where("idIndicador",$datos["idIndicador"])->get();
				foreach($evaluacion as $item)
				{
					$criterio = EvaluacionPCCriterio::find($item->id);
					$criterio->delete();
				}
				$hallazgo = Hallazgo::where("idEvaluacion",$id)->where("categoriaEvaluacion","PC")->where("idIndicador",$datos["idIndicador"])->get();
	...
	/**
	 * Devuelve la lista de criterios segun corresponda para el nivel de microred e indicador
	 *
	 * @param string $microred nivel de microred de la evaluación
	 ...
	 * <code> Respuesta Error json(array("status": 404, "messages": "No hay resultados"),status) </code>
	 */
	public function CriterioEvaluacion($microred,$indicador,$evaluacion)
	{		
		$datos = Request::all();
		
		
		$criterio = DB::select("SELECT c.id as idCriterio, c.tipo, c.habilitarNoAplica, c.tieneValidacion, ic.idIndicador, cic.idCone, lv.id as idlugarVerificacion, c.creadoAl, c.modificadoAl, c.nombre as criterio, lv.nombre as lugarVerificacion 
		FROM ConeIndicadorCriterio cic							
		left join IndicadorCriterio ic on ic.id = cic.idIndicadorCriterio
		left join Criterio c on c.id = ic.idCriterio
		left join LugarVerificacion lv on lv.id = ic.idlugarVerificacion		
		WHERE cic.idCone = 0 and ic.idIndicador = $indicador and c.borradoAl is null and ic.borradoAl is null and cic.borradoAl is null and lv.borradoAl is null order by c.orden ASC");	
			
		$evaluacionCriterio = EvaluacionPCCriterio::where('idEvaluacionPC',$evaluacion)->where('idIndicador',$indicador)->get();
		$aprobado=array();
		...
		else 
		{
			...
			left join Accion a on a.id = h.idAccion WHERE h.idEvaluacion= $evaluacion and categoriaEvaluacion='PC' and idIndicador='$indicador' and h.borradoAl is null");
			...
			foreach($criterio as $item){
				...
				$item->criterio_respuestas = CriterioValidacionRespuesta::where("idCriterio",$item->idCriterio)
											 ->where('idEvaluacion',$evaluacion)->where('tipo','PC')->get();
			}
	...
	{
		try{
			$clues = DB::select("SELECT clues FROM EvaluacionPC WHERE id=$evaluacion")[0]->clues;		
			$evaluacionCriterio = EvaluacionPCCriterio::with('Evaluaciones')->where('idEvaluacionPC',$evaluacion)->get(array('idCriterio','aprobado','id','idIndicador'));
			...
			foreach($evaluacionCriterio as $item)
			{
				...
				(SELECT count(id) FROM ConeIndicadorCriterio where borradoAl is null and idIndicadorCriterio in(select id from IndicadorCriterio where idIndicador=ci.idIndicador and borradoAl is null and idCriterio in (SELECT id FROM Criterio where borradoAl is null)) and idCone=0) as total, 
				(SELECT count(id) FROM EvaluacionPCCriterio where idEvaluacionPC = '$evaluacion' and idIndicador = '$item->idIndicador') as evaluado
				FROM Microred cc 
				left join ConeIndicadorCriterio cic on cic.idCone = 0
	...
	public function CriterioEvaluacionImprimir($microred,$indicador)
	{		
		$datos = Request::all();
